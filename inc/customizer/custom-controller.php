...
if(class_exists( 'WP_Customize_control')):
    ...
    */
	class Businessroy_Repeater_Control extends WP_Customize_Control{
	    ...
	    */
	    ...
	    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
		    $this->fields = $fields;
		    $this->cl_box_label = $args['cl_box_label'] ;
		    $this->cl_box_add_control = $args['cl_box_add_control'];
	    ...
	    public function render_content() {
		    ...
		    <ul class="cl-repeater-field-control-wrap">
		        <?php $this->cl_get_fields(); ?>
		    </ul>

	      	<input type="hidden" <?php esc_attr( $this->link() ); ?> class="cl-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />

	      	<button type="button" class="button cl-add-control-field">
	      		<?php echo esc_html( $this->cl_box_add_control ); ?>
	    ...
	    private function cl_get_fields(){
		    ...
		    if(is_array($values)){
		        ...
		        foreach($values as $value){
		        	?>
				    <li class="cl-repeater-field-control">

				        <h3 class="cl-repeater-field-title accordion-section-title">
				        	<?php echo esc_html( $this->cl_box_label ); ?>
				        </h3>

				        <div class="cl-repeater-fields">
				            <?php
				              foreach ($fields as $key => $field) {

				              $class = isset( $field['class'] ) ? $field['class'] : '';
				            ?>
				            <div class="cl-fields cl-type-<?php echo esc_attr( $field['type'] ).' '.esc_attr( $class ); ?>">
				                ...
				                <?php }
				                  ...
				                  switch ( $field['type'] ) {
				                    ...
				                    case 'icons':
				                        echo '<div class="cl-repeater-selected-icon">';
				                        echo '<i class="'.esc_attr($new_value).'"></i>';
				                        echo '<span><i class="fa fa-angle-down"></i></span>';
				                        echo '</div>';
				                        echo '<ul class="cl-repeater-icon-list clearfix">';
				                          $businessroy_awesome_icon_array = businessroy_awesome_icon_array();

				                          foreach ($businessroy_awesome_icon_array as $cl_awesome_icon) {
				                            $icon_class = $new_value == $cl_awesome_icon ? 'icon-active' : '';
				                            echo '<li class='.esc_attr( $icon_class ).'><i class="'.esc_attr( $cl_awesome_icon ).'"></i></li>';
				                          }
				                        echo '</ul>';
				                    ...
				                    case 'upload':
				                        ...
				                        echo '<div class="cl-fields-wrap">';
				                        echo '<div class="attachment-media-view">';
				                        echo '<div class="placeholder'.esc_attr( $image_class ).'">';
				                        esc_html_e('No image selected', 'business-roy');
				                        echo '</div>';
				                        echo '<div class="thumbnail thumbnail-image">';
				                        echo $image;
				                        echo '</div>';
				                        echo '<div class="actions clearfix">';
				                        echo '<button type="button" class="button cl-delete-button align-left">'.esc_html__('Remove', 'business-roy').'</button>';
				                        echo '<button type="button" class="button cl-upload-button alignright">'.esc_html__('Select Image', 'business-roy').'</button>';
				            ...
				            <div class="clearfix cl-repeater-footer">
					            <div class="alignright">
					                <a class="cl-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'business-roy') ?></a> |
					                <a class="cl-repeater-field-close" href="#close"><?php esc_html_e('Close', 'business-roy') ?></a>
					            </div>
				            </div>
				        </div>
				    </li>
	...
	/**
	  * Multiple Check
	*/
    class Businessroy_Multiple_Check_Control extends WP_Customize_Control {
    ...
    */
    class Businessroy_Switch_Control extends WP_Customize_Control{
...
*/
if(!function_exists('businessroy_awesome_icon_array') ){

	function businessroy_awesome_icon_array(){
